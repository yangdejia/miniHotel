...
function editPwd(){
	var str='<strong style="color:#333">Confirm password change?</strong>';
	var foot='<button class="btn btn-success" onClick="javascript:sureEditPwd();">Confirm</button><button class="btn btn-default" data-dismiss="modal">Close</button>';
	showMessage('tips',str,foot);
}
function sureEditPwd(){
	 $.ajax({
	       ...
	       beforeSend: function () {
	    	   showMessage("tips","<strong style='color:green'>Currently Modifying</strong>&nbsp;&nbsp;<img src='../../img/login.gif' style='width:40px;height:40px'>",null); //点击登录后显示loading，隐藏输入框
	    	   $('#myModalBody').addClass("text-center");
	       },
	       success: function (msg) {
	           if(msg=="success"){
	        	   showMessage("tips","<strong style='color:green'>The modification was successful！</strong>",null);  
	        	   setTimeout(closeMessage,800);
	        	   setTimeout("window.location.reload()",800);
	           }else if(msg == "fail"){
	        	   showError("Modification failed!");
	           }
	       },
...
function editPerson(){
	...
	var str='<strong style="color:#333">Confirm modification of personal information?</strong>';
...
function sureEditPerson(){
	...
	+"&address="+$('#userAddress').val()+"&remark="+$('#userRemark').val()+"&type="+2;
	 $.ajax({
	       ...
	       data:data,
	       beforeSend: function () {
	    	   showMessage("tips","<strong style='color:green'>Loading</strong>&nbsp;&nbsp;<img src='../../img/login.gif' style='width:40px;height:40px'>",null);
	    	   $('#myModalBody').addClass("text-center");
	       },
	       success: function (msg) {
	           if(msg=="success"){
	        	   personInfo();
	        	   showMessage("tips","<strong style='color:green'>Modification successful!</strong>",null);
