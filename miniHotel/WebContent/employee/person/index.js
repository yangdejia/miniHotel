...
function sureEditPwd(){
	 $.ajax({
	       ...
	       beforeSend: function () {
	    	   showMessage("prompt","<strong style='color:green'>Modifying</strong>&nbsp;&nbsp;<img src='../../img/login.gif' style='width:40px;height:40px'>",null); //点击登录后显示loading，隐藏输入框
...
function sureEditPerson(){
	...
	+"&address="+$('#userAddress').val()+"&remark="+$('#userRemark').val()+"&type="+0;
	 $.ajax({
	       ...
	       data:data,
	       beforeSend: function () {
	    	   showMessage("prompt","<strong style='color:green'>Loading</strong>&nbsp;&nbsp;<img src='../../img/login.gif' style='width:40px;height:40px'>",null); //点击登录后显示loading，隐藏输入框
	    	   $('#myModalBody').addClass("text-center");
	       },
	       success: function (msg) {
	           if(msg=="success"){
